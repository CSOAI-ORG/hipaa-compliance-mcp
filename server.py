...
def _rl(caller: str = "anonymous", tier: str = "free") -> Optional[str]:
    ...
    try:
        _meter = _server_meter_check("hipaa_compliance")
        if not _meter.get("allowed", True):
            return "Free tier limit reached. Upgrade to Pro at https://meok.ai/mcp/hipaa-compliance/pro"
    except Exception:
        pass  # fail-open
    if tier == "pro":
        return None
...
_MEOK_API_KEY_HIPAA = _meter_os.environ.get("MEOK_API_KEY", "") or _MEOK_API_KEY  # fall back to existing module-level
...
def _server_meter_check(tool: str) -> dict:
    ...
    if not _MEOK_API_KEY_HIPAA:
        return {"allowed": True, "tier": "anon", "note": "MEOK_API_KEY not set; metering skipped"}
    try:
        body = _meter_json.dumps({"api_key": _MEOK_API_KEY_HIPAA, "tool": tool}).encode()
